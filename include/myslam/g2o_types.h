...
/// 位姿顶点
    class VertexPose : public g2o::BaseVertex<6, cv::Mat> {
    public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

        virtual void setToOriginImpl() override { _estimate = cv::Mat(); }

        /// left multiplication on cv::Mat
        virtual void oplusImpl(const double *update) override {
            Vec6 update_eigen;
            update_eigen << update[0], update[1], update[2], update[3], update[4],
                    update[5];
            _estimate = cv::Mat::exp(update_eigen) * _estimate;
        }
...
/// 带有地图和位姿的二元边
    ...
    public:
        ...
        /// 构造时传入相机内外参
        EdgeProjection(const Mat33 &K, const cv::Mat &cam_ext) : _K(K) {
            _cam_ext = cam_ext;
        }
